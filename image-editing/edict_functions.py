...
def CBDIA_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = CBDIA_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                     init_image_strength=init_image_strength,
                                      steps=steps,
                                     ...
                                     run_baseline=run_baseline)
    # Denoise intermediate state with new conditioning
    gen = CBDIA_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                steps=steps,
...
def BDIA_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIA_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                     init_image_strength=init_image_strength,
                                      steps=steps,
                                     ...
                                     run_baseline=run_baseline)
    # Denoise intermediate state with new conditioning
    gen = BDIA_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                steps=steps,
...
def BDIAV2_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV2_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                     init_image_strength=init_image_strength,
                                      steps=steps,
                                     ...
                                     run_baseline=run_baseline)
    # Denoise intermediate state with new conditioning
    gen = BDIAV2_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                steps=steps,
...
def BDIAV3_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV3_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                     init_image_strength=init_image_strength,
                                      gamma=gamma,
                                     ...
                                     run_baseline=run_baseline)
    # Denoise intermediate state with new conditioning
    gen = BDIAV3_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                gamma=gamma,
...
def BDIAV5_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV5_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                      init_image_strength=init_image_strength,
                                      gamma=gamma,
    ...
    # Denoise intermediate state with new conditioning
    gen = BDIAV5_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                 fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                 gamma=gamma,
...
def BDIAV6_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV6_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                      init_image_strength=init_image_strength,
                                      gamma=gamma,
    ...
    recon = BDIAV6_stablediffusion(base_prompt,
                                   reverse=False,
                                    fixed_starting_latent=latents,
                                    steps=steps,
    ...
    # Denoise intermediate state with new conditioning
    gen = BDIAV6_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                 fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                 gamma=gamma,
...
def BDIAV7_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV7_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                      init_image_strength=init_image_strength,
                                      gamma=gamma,
    ...
    # Denoise intermediate state with new conditioning
    gen = BDIAV7_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                 fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                 gamma=gamma,
...
def BDIAV8_editing(im_path,
    ...
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = BDIAV8_stablediffusion(base_prompt,
                                     reverse=True,
                                      init_image=orig_im,
                                      init_image_strength=init_image_strength,
                                      gamma=gamma,
    ...
    # Denoise intermediate state with new conditioning
    gen = BDIAV8_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                  None if (not use_p2p) else edit_prompt,
                                 fixed_starting_latent=latents,
                                 init_image_strength=init_image_strength,
                                 gamma=gamma,
...
def forward_step(
    ...
    #print("timestep, prev_timestep ", timestep, prev_timestep)
...
@torch.no_grad()
def CBDIA_stablediffusion(prompt="",
                           prompt_edit=None,
                            null_prompt='',
                            prompt_edit_token_weights=[],
                            prompt_edit_tokens_start=0.0,
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        ...
        for latent_i in range(2): 
            ...
            if not reverse and latent_i==0:
              t_tmp = t
            elif not reverse and latent_i==1: 
              t_tmp = t - schedulers[0].config.num_train_timesteps / schedulers[0].num_inference_steps 
            elif reverse and latent_i==0:
              t_tmp = t - schedulers[0].config.num_train_timesteps / schedulers[0].num_inference_steps
            elif reverse and latent_i==1:
              t_tmp = t
            #Predict the unconditional noise residual
            noise_pred_uncond = unet(latent_model_input, t_tmp, 
                                     encoder_hidden_states=embedding_unconditional).sample
            ...
            #Predict the conditional noise residual and save the cross-attention layer activations
            noise_pred_cond = unet(latent_model_input, t_tmp, 
                                   encoder_hidden_states=embedding_conditional).sample

            #Edit the Cross-Attention layer activations
            if prompt_edit is not None:
                t_scale = t_tmp / schedulers[0].num_train_timesteps
                if t_scale >= prompt_edit_tokens_start and t_scale <= prompt_edit_tokens_end:
                    use_last_tokens_attention()
                if t_scale >= prompt_edit_spatial_start and t_scale <= prompt_edit_spatial_end:
                    use_last_self_attention()
            ...
            if reverse and latent_i==0:
              t_cur = t - schedulers[0].config.num_train_timesteps / schedulers[0].num_inference_steps 
              t_next= t             
              alpha_prod_t_cur, beta_prod_t_cur = get_alpha_and_beta(t_cur, schedulers[0])
              alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, schedulers[0])
              
              alpha_quotient = ((alpha_prod_t_next / alpha_prod_t_cur)**0.5)
              first_term =  alpha_quotient * latent_model_input
              second_term = ((beta_prod_t_next)**0.5) * noise_pred
              third_term = alpha_quotient * ((1 - alpha_prod_t_cur)**0.5) * noise_pred
              
              new_latent =latent_base+ (first_term + second_term - third_term-latent_model_input)               
            elif reverse and latent_i==1:
              t_cur = t - schedulers[0].config.num_train_timesteps / schedulers[0].num_inference_steps 
              t_next= t    
              alpha_prod_t_cur, beta_prod_t_cur = get_alpha_and_beta(t_cur, schedulers[0])
              ...
              alpha_quotient = ((alpha_prod_t_cur / alpha_prod_t_next)**0.5)
              first_term =  alpha_quotient * latent_model_input
              second_term = ((beta_prod_t_cur)**0.5) * noise_pred
              third_term = alpha_quotient * ((1 - alpha_prod_t_next)**0.5) * noise_pred           
              new_latent =latent_base- (first_term + second_term - third_term-latent_model_input)   

            elif not reverse and latent_i==0:
              t_cur= t
              t_next = t - schedulers[0].config.num_train_timesteps / schedulers[0].num_inference_steps 
              alpha_prod_t_cur, beta_prod_t_cur = get_alpha_and_beta(t_cur, schedulers[0])
              alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, schedulers[0])
...
##### MAIN BDIA-DDIM FUNCTION #######
# Use BDIA_editing to perform calls

@torch.no_grad()
def BDIA_stablediffusion(prompt="",
                          prompt_edit=None,
                          null_prompt='',
                          prompt_edit_token_weights=[],
                          prompt_edit_tokens_start=0.0,
    ...
    if not reverse:
      latent_prev = latent_pair[0]
      new_latent = latent_pair[1]
      t_prev = timesteps[0]
      timesteps = timesteps[1:]
    else:   
      latent_prev = None
      t_prev = None
      new_latent = latent_pair[0] 

    for i, t in tqdm(enumerate(timesteps[:-1]), total=len(timesteps[:-1])):
        #t_scale = t / scheduler.num_train_timesteps
        latent_model_input = new_latent
      
        #if i==0: #compute z_1 by running DDIM Inversion 
        ...
        #step_call = reverse_step if reverse else forward_step
        if reverse: #from image to noisy representation
          t_next = timesteps[i+1]          
          alpha_prod_t, beta_prod_t = get_alpha_and_beta(t, scheduler)
          alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, scheduler)
          #print(i, t, t_next)
          #print(alpha_prod_t+beta_prod_t)
          #print(alpha_prod_t_next+beta_prod_t_next)
          ...
          if t_prev is not None:
            #print(i, t_prev)
            #input()
            #perform reverse DDIM
            alpha_prod_t_prev, beta_prod_t_prev = get_alpha_and_beta(t_prev, scheduler)
            alpha_quotient_prev = ((alpha_prod_t_prev / alpha_prod_t)**0.5)
            first_term_prev =  alpha_quotient_prev * latent_model_input
            second_term_prev = ((beta_prod_t_prev)**0.5) * noise_pred
            third_term_prev = alpha_quotient_prev * ((1 - alpha_prod_t)**0.5) * noise_pred
            
            new_latent = (first_term + second_term - third_term
                          -1*(latent_model_input-latent_prev) 
                          +1*(latent_model_input-(first_term_prev+second_term_prev-third_term_prev)) 
                        )
          else:
            new_latent=first_term + second_term - third_term

        else: #from noisy representation to image
          t_next = timesteps[i+1]          
          alpha_prod_t, beta_prod_t = get_alpha_and_beta(t, scheduler)
          alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, scheduler)
          ...
          if t_prev is not None:
            ...
            new_latent = (1*latent_prev
                        - 1*(first_term_prev+second_term_prev-third_term_prev)
...
@torch.no_grad()
def BDIAV2_stablediffusion(prompt="",
                          prompt_edit=None,
                          null_prompt='',
                          prompt_edit_token_weights=[],
                          prompt_edit_tokens_start=0.0,
    ...
    for i, t in tqdm(enumerate(timesteps[:-1]), total=len(timesteps[:-1])):
        ...
        if (i==0 and not reverse):
          ###########################
          noise_pred_uncond_prev = unet(latent_prev, t_prev, 
                                    encoder_hidden_states=embedding_unconditional).sample
          ...
          #Predict the conditional noise residual and save the cross-attention layer activations
          noise_pred_cond_prev = unet(latent_prev, t_prev, 
                                  encoder_hidden_states=embedding_conditional).sample

          #Edit the Cross-Attention layer activations
          if prompt_edit is not None:
              t_scale_prev = t_prev / scheduler.num_train_timesteps
              if t_scale_prev >= prompt_edit_tokens_start and t_scale_prev <= prompt_edit_tokens_end:
                  use_last_tokens_attention()
              if t_scale_prev >= prompt_edit_spatial_start and t_scale_prev <= prompt_edit_spatial_end:
                  use_last_self_attention()
              ...
              #Predict the edited conditional noise residual using the cross-attention masks
              noise_pred_cond_prev = unet(latent_prev,
                                      t_prev, 
                                      encoder_hidden_states=embedding_conditional_edit).sample

          #Perform guidance
          grad_prev = (noise_pred_cond_prev - noise_pred_uncond_prev)
          noise_pred_prev = noise_pred_uncond_prev + guidance_scale * grad_prev
          #################
        ...
        if reverse: #from image to noisy representation
          ...
          if t_prev is not None:
            ...
            new_latent = (latent_prev/gamma
                          +(first_term + second_term - third_term)
                          -(first_term_prev+second_term_prev-third_term_prev)/gamma 
                        )
          ...
          if i==len(timesteps[:-1])-1:
            ###########################
            noise_pred_uncond_last = unet(new_latent, t_next, 
                                      encoder_hidden_states=embedding_unconditional).sample
            ...
            #Predict the conditional noise residual and save the cross-attention layer activations
            noise_pred_cond_last = unet(new_latent, t_next, 
                                    encoder_hidden_states=embedding_conditional).sample

            #Edit the Cross-Attention layer activations
            if prompt_edit is not None:
                t_scale_last = t_next / scheduler.num_train_timesteps
                if t_scale_last >= prompt_edit_tokens_start and t_scale_last <= prompt_edit_tokens_end:
                    use_last_tokens_attention()
                if t_scale_last >= prompt_edit_spatial_start and t_scale_last <= prompt_edit_spatial_end:
                    use_last_self_attention()
                ...
                #Predict the edited conditional noise residual using the cross-attention masks
                noise_pred_cond_last = unet(new_latent,
                                        t_next, 
                                        encoder_hidden_states=embedding_conditional_edit).sample

            #Perform guidance
            grad_last = (noise_pred_cond_last - noise_pred_uncond_last)
            noise_pred_last = noise_pred_uncond_last + guidance_scale * grad_last
            #################
            alpha_quotient_last_B = ((alpha_prod_t / alpha_prod_t_next)**0.5)
            first_term_last_B =  alpha_quotient_last_B * new_latent
            second_term_last_B = ((beta_prod_t)**0.5) * noise_pred_last
            third_term_last_B = alpha_quotient_last_B * ((1 - alpha_prod_t_next)**0.5) * noise_pred_last
            
            latent_model_input = (latent_model_input 
                                  -(first_term_last_B+second_term_last_B-third_term_last_B)
                                )
          ...
          if t_prev is not None:
            ...
            if i==0:
              alpha_quotient_prev_F = ((alpha_prod_t/alpha_prod_t_prev )**0.5)
              first_term_prev_F =  alpha_quotient_prev_F * latent_prev
              second_term_prev_F = ((beta_prod_t)**0.5) * noise_pred_prev
              third_term_prev_F = alpha_quotient_prev_F * ((1 - alpha_prod_t_prev)**0.5) * noise_pred_prev

              alpha_quotient_prev = ((alpha_prod_t_prev / alpha_prod_t)**0.5)
              first_term_prev_B =  alpha_quotient_prev * latent_model_input
              second_term_prev_B = ((beta_prod_t_prev)**0.5) * noise_pred
              third_term_prev_B = alpha_quotient_prev * ((1 - alpha_prod_t)**0.5) * noise_pred

              gamma=1.0
              #new_latent = (latent_prev
              #              +gamma*(first_term_prev_F+second_term_prev_F-third_term_prev_F-latent_prev)
              #              -gamma*(first_term_prev_B + second_term_prev_B - third_term_prev_B-latent_model_input)
              #              +(first_term+second_term-third_term-latent_model_input)
              #            )
              new_latent = (-gamma*(first_term_prev_F+second_term_prev_F-third_term_prev_F-latent_prev)
                +(first_term + second_term - third_term)
                - gamma*(first_term_prev_B+second_term_prev_B-third_term_prev_B-latent_model_input)
                ) 

              #gamma=0.5
              #new_latent = (gamma*latent_prev
              #  +(first_term + second_term - third_term)
              #  - gamma*(first_term_prev_B+second_term_prev_B-third_term_prev_B)
              #  )                              
            else:
              #perform reverse DDIM
              alpha_quotient_prev = ((alpha_prod_t_prev / alpha_prod_t)**0.5)
              ...
              new_latent = (gamma*latent_prev
                          +(first_term + second_term - third_term)
                          - gamma*(first_term_prev+second_term_prev-third_term_prev)
                          )            
          else:
            new_latent=first_term + second_term - third_term
...
@torch.no_grad()
def BDIAV3_stablediffusion(prompt="",
                          prompt_edit=None,
                          null_prompt='',
                          prompt_edit_token_weights=[],
                          prompt_edit_tokens_start=0.0,
                          ...
                          prompt_edit_spatial_start=0.0,
                          prompt_edit_spatial_end=1.0,
                          guidance_scale=7.0, gamma=1.0, steps=50,
                          seed=1, width=512, height=512,
                          init_image=None, init_image_strength=1.0,
                          run_baseline=False,
                          use_lms=False,
...
@torch.no_grad()
def BDIAV5_stablediffusion(prompt="",
                           prompt_edit=None,
                            null_prompt='',
                            prompt_edit_token_weights=[],
                            prompt_edit_tokens_start=0.0,
    ...
    if reverse: 
      ...
      #the following setup does not work well
      #latent_prev = None
      #new_latent = latent_pair[0]
      #t_prev = None
...
@torch.no_grad()
def BDIAV7_stablediffusion(prompt="",
                           prompt_edit=None,
                            null_prompt='',
                            prompt_edit_token_weights=[],
                            prompt_edit_tokens_start=0.0,
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        ...
        latent_model_input = latent_pair[0] #new_latent
        if reverse:
          t_cur = t - scheduler.config.num_train_timesteps / scheduler.num_inference_steps
          t_next = t
        else:
          t_cur = t
          t_next = t - scheduler.config.num_train_timesteps / scheduler.num_inference_steps
        
        #Predict the unconditional noise residual
        noise_pred_uncond = unet(latent_model_input, t_cur, 
                                  encoder_hidden_states=embedding_unconditional).sample
        ...
        #Predict the conditional noise residual and save the cross-attention layer activations
        noise_pred_cond = unet(latent_model_input, t_cur, 
                                encoder_hidden_states=embedding_conditional).sample

        #Edit the Cross-Attention layer activations
        if prompt_edit is not None:
            t_scale = t_cur / scheduler.num_train_timesteps
            if t_scale >= prompt_edit_tokens_start and t_scale <= prompt_edit_tokens_end:
                use_last_tokens_attention()
            if t_scale >= prompt_edit_spatial_start and t_scale <= prompt_edit_spatial_end:
                use_last_self_attention()
        ...
        if reverse and t_prev==None:
          ...
          alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, scheduler)
          alpha_prod_t, beta_prod_t = get_alpha_and_beta(t_cur, scheduler)
          alpha_quotient = ((alpha_prod_t_next / alpha_prod_t)**0.5)
          first_term =  alpha_quotient * latent_model_input
          second_term = ((beta_prod_t_next)**0.5) * noise_pred
          third_term = alpha_quotient * ((1 - alpha_prod_t)**0.5) * noise_pred
          ...
          step_call = reverse_step if reverse else forward_step
          new_latent = step_call(scheduler,
                                    noise_pred,
                                      t_next,
                                      latent_model_input)# .prev_sample
          new_latent = new_latent.to(latent_model_input.dtype)
          latent_backward = forward_step(scheduler,
                                      noise_pred,
                                      t_cur,
                                      latent_model_input)# .prev_sample
          latent_backward = latent_backward.to(latent_model_input.dtype)
...
@torch.no_grad()
def BDIAV8_stablediffusion(prompt="",
                           prompt_edit=None,
                            null_prompt='',
                            prompt_edit_token_weights=[],
                            prompt_edit_tokens_start=0.0,
    ...
    '''
    if not reverse:
      tmp  = 0.5*latent_pair[0]+0.5*latent_pair[1]
      latent_pair[0] = tmp.clone()
      latent_pair[1] = tmp.clone()
    '''

    # compute noise_pred_cur_y
    if reverse:
      t_cur = timesteps[0] - scheduler.config.num_train_timesteps / scheduler.num_inference_steps
    else:
      t_cur = timesteps[0]
    #Predict the unconditional noise residual
    noise_pred_uncond = unet(latent_pair[0], t_cur, 
                              encoder_hidden_states=embedding_unconditional).sample
    #Prepare the Cross-Attention layers
    if prompt_edit is not None:
        save_last_tokens_attention()
    ...
    #Predict the conditional noise residual and save the cross-attention layer activations
    noise_pred_cond = unet(latent_pair[0], t_cur, 
                            encoder_hidden_states=embedding_conditional).sample
    #Perform guidance
    grad = (noise_pred_cond - noise_pred_uncond)
    noise_pred_cur_y = noise_pred_uncond + guidance_scale * grad


    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        if i%2 ==1:
          latent_pair[0], latent_pair[1] = latent_pair[1], latent_pair[0]        
        '''
        if reverse:
          if i%2 ==1:
            latent_pair[0], latent_pair[1] = latent_pair[1], latent_pair[0]
        else:
          if len(timesteps)%2==0:
            if i%2 ==1:
              latent_pair[0], latent_pair[1] = latent_pair[1], latent_pair[0]            
          else:
            if i%2 ==0:
        ...
        alpha_quotient = ((alpha_prod_t_next / alpha_prod_t)**0.5)
        first_term =  alpha_quotient * latent_model_input
        second_term = ((beta_prod_t_next)**0.5) * noise_pred_cur_y
        third_term = alpha_quotient * ((1 - alpha_prod_t)**0.5) * noise_pred_cur_y

        if reverse:
          v = latent_pair[1]/gamma - (1-gamma)/gamma*latent_pair[0]
          y_new = v + (first_term + second_term - third_term -latent_pair[0])
        else:
          #v = latent_pair[1]/(1+gamma) - gamma/(1-gamma)*latent_pair[0]
          y_new = latent_pair[1] + (first_term + second_term - third_term -latent_pair[0])
          
        #Predict the unconditional noise residual
        noise_pred_uncond = unet(y_new, t_next, 
                                encoder_hidden_states=embedding_unconditional).sample
        #Prepare the Cross-Attention layers
        if prompt_edit is not None:
            save_last_tokens_attention()
        ...
        #Predict the conditional noise residual and save the cross-attention layer activations
        noise_pred_cond = unet(y_new, t_next, 
                                encoder_hidden_states=embedding_conditional).sample
        ...
        #alpha_prod_t_next, beta_prod_t_next = get_alpha_and_beta(t_next, scheduler)
        #alpha_prod_t, beta_prod_t = get_alpha_and_beta(t_cur, scheduler)
        alpha_quotient_reverse = ((alpha_prod_t / alpha_prod_t_next)**0.5)
        first_term =  alpha_quotient_reverse * y_new
        second_term = ((beta_prod_t)**0.5) * noise_pred_cur_y
        third_term = alpha_quotient_reverse * ((1 - alpha_prod_t_next)**0.5) * noise_pred_cur_y
                
        if reverse:
          z_new = latent_pair[0]-(first_term + second_term - third_term-y_new)
        else:
          v = latent_pair[0]-(first_term + second_term - third_term-y_new)
          z_new = (1-gamma)*y_new + gamma*v         

        latent_pair[0] = y_new  
        latent_pair[1] = z_new  

    #if not reverse:
      #latent_pair[0] = latent_pair[1].clone()
    #  latent_pair[0] = 0.5*latent_pair[0]+0.5*latent_pair[1]
