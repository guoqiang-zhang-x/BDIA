...
from ldm.models.diffusion.BDIAddim import BDIADDIMSampler
...
def parse_args():
    ...
    parser.add_argument(
        "--bdiaddim",
        action='store_true',
        help="use BDIAddim sampler",
    )  
    parser.add_argument(
        "--gamma",
        type=float,
        default=0.5,
        help="gamma parameter needed in BDIADDIM",
    )  
...
def main(opt):
    ...
    if opt.plms:
        ...
        sampler = BDIADDIMSampler(model, num_steps=opt.steps, gamma=opt.gamma, device=device)
    else:
        ValueError("sampling method is not specified")
